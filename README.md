...
AndroidValidatedForms is a library that allows you to dynamically generate forms in Android with little code. It's great
...
AndroidValidatedForms 1.0.0 will be the first stable release.

A Simple Example
----------------
To give you an idea of the simplicity of AndroidValidatedForms, here's a screenshot of a simple example:
...
Here's the code for that example (less than 7 lines of real code!):
    ...
    import com.azavea.androidvalidatedforms.FormActivity;
    import com.azavea.androidvalidatedforms.controllers.*;
...
For more examples, browse the [sample](http://github.com/azavea/AndroidValidatedForms/tree/master/sample) directory.

Features
--------
AndroidValidatedForms supports many built-in fields for your form, like text boxes, date pickers, spinners, etc. The framework is
...
How to Add AndroidValidatedForms to Your Project
--------------------------------------
...
Add the following dependency to your build.gradle file for your project:
    ...
    dependencies {
      compile 'com.azavea.androidvalidatedforms:androidvalidatedforms:1.0.0'
    }
...
Add the following dependency to your pom.xml file for your project (requires android-maven-plugin 3.8.0+):
    ...
    <dependency>
        <groupId>com.azavea.androidvalidatedforms</groupId>
        <artifactId>androidvalidatedforms</artifactId>
...
How to Use AndroidValidatedForms
----------------------
Once AndroidValidatedForms is setup as a dependency in your project (by following the instructions above), you can start creating
forms right away! The main classes you will be working with the most are:

* `FormActivity` or `FormActivityWithActionBar`: If you wish to use the default Activity implementation for AndroidValidatedForms,
...
* `FormController`: This is the main class that manages the form elements of AndroidValidatedForms. It provides simple APIs to
...
* `FormModel`: This class abstracts the data model your form will use. It's the interface that AndroidValidatedForms uses to
...
Please browse through the samples included with the project for examples on how AndroidValidatedForms can be used.
...
----------------------
If the built-in form controls provided by AndroidValidatedForms don't meet your needs, you can easily extend AndroidValidatedForms to
provide custom form elements. The common parent class for all form elements is `FormElementController`. Among other
things, `FormElementController` tells AndroidValidatedForms how to construct the view to display in the form.
...
2. Your custom element needs full customization for how it's displayed: in this case, inherit from
   `FormElementController` and implement the `createView` method to tell AndroidValidatedForms how to create the custom view.
